...
public abstract class FluidMoveBehaviorBase : Behavior<FrameworkElement>
{
	...
	private void AssociatedObject_LayoutUpdated(object sender, EventArgs e)
	{
		...
		{
			if (!(base.AssociatedObject is Panel panel))
			{
				return;
			}
	...
	{
		if (Tag == TagType.DataContext && !(child.ReadLocalValue(IdentityTagProperty) is BindingExpression))
		{
	...
	private static FrameworkElement GetVisualRoot(FrameworkElement child)
	{
		while (true)
		{
			if (!(VisualTreeHelper.GetParent(child) is FrameworkElement frameworkElement))
			{
				return child;
			}
