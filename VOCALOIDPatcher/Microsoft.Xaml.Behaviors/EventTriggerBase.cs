...
public abstract class EventTriggerBase : TriggerBase
{
    ...
    protected sealed override Type AssociatedObjectTypeConstraint
    {
        get
        {
            ...
            var typeConstraintAttribute = attributes[typeof(TypeConstraintAttribute)] as TypeConstraintAttribute;

            if (typeConstraintAttribute != null) return typeConstraintAttribute.Constraint;
    ...
    private bool IsSourceNameSet => !string.IsNullOrEmpty(SourceName) ||
                                    ReadLocalValue(SourceNameProperty) != DependencyProperty.UnsetValue;
    ...
    {
        if (AssociatedObject != null) OnSourceChangedImpl(oldSource, newSource);
    }
    ...
    {
        if (string.IsNullOrEmpty(GetEventName())) return;
    ...
    protected override void OnAttached()
    {
        ...
        base.OnAttached();
        var newHost = AssociatedObject;
        var newBehavior = newHost as Behavior;
        var newHostElement = newHost as FrameworkElement;

        RegisterSourceChanged();
        if (newBehavior != null)
        {
            newHost = ((IAttachedObject)newBehavior).AssociatedObject;
            newBehavior.AssociatedObjectChanged += OnBehaviorHostChanged;
        }
        ...
        var isLoadedEvent = string.Compare(GetEventName(), "Loaded", StringComparison.Ordinal) == 0;
    ...
    {
        base.OnDetaching();
        var oldBehavior = AssociatedObject as Behavior;
        var oldElement = AssociatedObject as FrameworkElement;
        ...
        if (oldBehavior != null) oldBehavior.AssociatedObjectChanged -= OnBehaviorHostChanged;

        SourceNameResolver.NameScopeReferenceElement = null;

        var isLoadedEvent = string.Compare(GetEventName(), "Loaded", StringComparison.Ordinal) == 0;

        if (isLoadedEvent && oldElement != null) UnregisterLoaded(oldElement);
    }
    ...
    {
        var trigger = (EventTriggerBase)obj;
    ...
    {
        if (SourceObject == null) OnSourceChanged(e.OldObject, e.NewObject);
    }
    ...
    private void RegisterEvent(object obj, string eventName)
    {
        ...
        eventInfo.AddEventHandler(obj,
            Delegate.CreateDelegate(eventInfo.EventHandlerType, this, eventHandlerMethodInfo));
    }
    ...
    private void UnregisterEvent(object obj, string eventName)
    {
        if (string.Compare(eventName, "Loaded", StringComparison.Ordinal) == 0)
        {
            ...
            if (frameworkElement != null) UnregisterLoaded(frameworkElement);
        }
        else
        {
    ...
    private void UnregisterEventImpl(object obj, string eventName)
    {
        ...
        if (eventHandlerMethodInfo == null) return;
    ...
    internal void OnEventNameChanged(string oldEventName, string newEventName)
    {
        if (AssociatedObject != null)
        {
            ...
            else if (!string.IsNullOrEmpty(oldEventName)) UnregisterEvent(Source, oldEventName);
            if (associatedElement != null && string.Compare(newEventName, "Loaded", StringComparison.Ordinal) == 0)
                RegisterLoaded(associatedElement);
            else if (!string.IsNullOrEmpty(newEventName)) RegisterEvent(Source, newEventName);
        }
    }
}
