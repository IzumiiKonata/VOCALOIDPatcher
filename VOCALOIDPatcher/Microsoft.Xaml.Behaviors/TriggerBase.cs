    ...
    IAttachedObject
{
    ...
    protected void InvokeActions(object parameter)
    {
        if (PreviewInvoke != null)
        {
            ...
            var previewInvokeEventArg = new PreviewInvokeEventArgs();
            PreviewInvoke(this, previewInvokeEventArg);
            // If a handler has cancelled the event, abort the invoke
            if (previewInvokeEventArg.Cancelling) return;
        }

        foreach (var action in Actions) action.CallInvoke(parameter);
    }
    ...
    public void Attach(DependencyObject dependencyObject)
    {
        if (dependencyObject != AssociatedObject)
        {
            ...
            if (dependencyObject != null &&
                !AssociatedObjectTypeConstraint.IsAssignableFrom(dependencyObject.GetType()))
