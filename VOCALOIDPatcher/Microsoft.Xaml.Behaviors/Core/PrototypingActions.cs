...
public abstract class PrototypingActionBase : TriggerAction<DependencyObject>
{
    ...
    protected UserControl GetContainingScreen()
    {
        ...
        var screen =
            userControlAncestors.FirstOrDefault(userControl =>
...
public sealed class ActivateStateAction : PrototypingActionBase
{
    ...
    {
        get => GetValue(TargetScreenProperty) as string;
        set => SetValue(TargetScreenProperty, value);
    }

    public string TargetState
    {
        get => GetValue(TargetStateProperty) as string;
    ...
    protected override void Invoke(object parameter)
    {
        ...
        if (string.IsNullOrEmpty(screen)) screen = GetContainingScreen().GetType().ToString();
...
public sealed class NavigateToScreenAction : PrototypingActionBase
{
    ...
    protected override void Invoke(object parameter)
    {
        ...
        var hostControl = AssociatedObject.GetSelfAndAncestors().OfType<UserControl>().FirstOrDefault();
        if (hostControl != null) libraryAssembly = hostControl.GetType().Assembly;
...
public sealed class PlaySketchFlowAnimationAction : PrototypingActionBase
{
    ...
    {
        get => GetValue(SketchFlowAnimationProperty) as string;
...
public sealed class NavigationMenuAction : TargetedTriggerAction<FrameworkElement>
{
    ...
    private bool IsTargetObjectSet
    {
        get
        {
            var isLocallySet = ReadLocalValue(TargetObjectProperty) != DependencyProperty.UnsetValue;
    ...
    protected override void OnTargetChanged(FrameworkElement oldTarget, FrameworkElement newTarget)
    {
        ...
        if (string.IsNullOrEmpty(TargetName) && !IsTargetObjectSet)
            VisualStateUtilities.TryFindNearestStatefulControl(AssociatedObject as FrameworkElement,
    ...
    protected override void Invoke(object parameter)
    {
        if (AssociatedObject != null) InvokeImpl(StateTarget);
    }
    ...
    internal void InvokeImpl(FrameworkElement stateTarget)
    {
        ...
        {
            var screen = stateTarget
                .GetSelfAndAncestors()
                .OfType<UserControl>()
                .FirstOrDefault(control => control.GetType().ToString() == TargetScreen);

            var stateName = InactiveState;
            if (screen != null) stateName = ActiveState;

            if (!string.IsNullOrEmpty(stateName))
            {
                var toggleButton = stateTarget as ToggleButton;
...
public sealed class RemoveItemInListBoxAction : TriggerAction<FrameworkElement>
{
    private ListBoxItem ItemContainer
    {
        get
        {
            return (ListBoxItem)AssociatedObject.GetSelfAndAncestors()
    ...
    private ItemsControl ItemsControl
    {
        get
        {
            return (ItemsControl)AssociatedObject.GetSelfAndAncestors()
    ...
    protected override void Invoke(object parameter)
    {
        ...
        if (items != null)
        {
            if (items.ItemsSource != null)
            {
                ...
                var list = items.ItemsSource as IList;
            ...
            else
            {
                ...
                if (listBox != null)
                {
                    var listBoxItem = ItemContainer;
                    if (listBoxItem != null) listBox.Items.Remove(listBoxItem.Content);
                }
            }
        }
    }
