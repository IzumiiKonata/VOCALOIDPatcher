    ...
    ICondition
{
    ...
    public static readonly DependencyProperty ForwardChainingProperty = DependencyProperty.Register("ForwardChaining",
        typeof(ForwardChaining), typeof(ConditionalExpression), new PropertyMetadata(ForwardChaining.And));
    ...
    public bool Evaluate()
    {
        ...
        foreach (var operation in Conditions)
        {
            ...
            if (!result && ForwardChaining == ForwardChaining.And) return result;

            if (result && ForwardChaining == ForwardChaining.Or) return result;
        }
