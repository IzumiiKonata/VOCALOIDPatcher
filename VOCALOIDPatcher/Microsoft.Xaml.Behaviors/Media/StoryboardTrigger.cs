...
/// </summary>
/// <remarks>
///     For Trigger authors, this class provides a standard way to target a Storyboard. Design tools may choose to provide
///     a
...
public abstract class StoryboardTrigger : TriggerBase<DependencyObject>
{
    ...
    {
        var storyboardTrigger = sender as StoryboardTrigger;
        if (storyboardTrigger != null) storyboardTrigger.OnStoryboardChanged(args);
    }
...
public class StoryboardCompletedTrigger : StoryboardTrigger
{
    ...
    {
        base.OnDetaching();
        if (Storyboard != null) Storyboard.Completed -= Storyboard_Completed;
    }

    protected override void OnStoryboardChanged(DependencyPropertyChangedEventArgs args)
    {
        var oldStoryboard = args.OldValue as Storyboard;
        var newStoryboard = args.NewValue as Storyboard;

        if (oldStoryboard != newStoryboard)
        {
            if (oldStoryboard != null) oldStoryboard.Completed -= Storyboard_Completed;
            if (newStoryboard != null) newStoryboard.Completed += Storyboard_Completed;
        }
    }
