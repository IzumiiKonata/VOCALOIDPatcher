...
public static class VisualStateUtilities
{
    ...
    ///     True to use a System.Windows.VisualTransition to transition between states; otherwise,
    ///     false.
    /// </param>
    ...
    public static bool GoToState(FrameworkElement element, string stateName, bool useTransitions)
    {
        ...
        {
            var targetControl = element as Control;
    ...
    public static IList GetVisualStateGroups(FrameworkElement targetObject)
    {
        ...
        if (targetObject != null)
        {
            ...
            {
                var childrenCount = VisualTreeHelper.GetChildrenCount(targetObject);
                if (childrenCount > 0)
                {
                    var childElement = VisualTreeHelper.GetChild(targetObject, 0) as FrameworkElement;
            ...
            if (visualStateGroups.Count == 0)
            {
                var userControl = targetObject as UserControl;
                if (userControl != null)
                {
                    var contentElement = userControl.Content as FrameworkElement;
    ...
    private static bool ShouldContinueTreeWalk(FrameworkElement element)
    {
        ...
        if (element.Parent == null)
        {
            ...
            var templatedParent = FindTemplatedParent(element);
            if (templatedParent == null ||
                (!(templatedParent is Control) && !(templatedParent is ContentPresenter))) return false;
        }
